...
export default function MediaCard({ item, streamUrl, onClick }) {
  // Only attempt thumbnail for video items
  const { thumbnail, loading } = useThumbnail(
    item.type === 'video' ? streamUrl : null
  )
  ...
  return (
    <article
      ...
      <div className="relative aspect-video overflow-hidden bg-card">
        ...
        {/* Duration */}
        {item.duration && (
          <span className="absolute bottom-2 right-2 text-xs bg-black/70 text-white
      ...
      <div className="p-4">
        <h3 className="font-display font-semibold text-white text-base leading-tight
                       line-clamp-1 mb-1 group-hover:text-accent transition-colors">
          {item.title}
        </h3>
